...
// Preparation    
    int startfd;
    startfd = open("time_test", O_RDWR | O_TRUNC | O_CREAT, 0777); 
    write(startfd,"NONE\n",10);
...
//// HANDLE 400 ////
    ...
    if(strncmp(query,checkcommand,9)==0 && strcmp(file,"file_reader")==0){
        ...
        if(fork()==0){
            if(execlp("./mmap_write","./mmap_write",pwdfilename,(char *)0)<0) 
        ...
        if(pid==0){
            close(rpfd[0]);
            dup2(rpfd[1],1);
            if(execlp("./file_reader","./file_reader",pwdfilename,(char *)0)<0) 
                fprintf(stderr, "exec error\n");
            fprintf(stderr, "exec error\n" );
        }
        else{
...
//// HANDLE INFO ////    
        ...
        if(fork()==0){
            ...
            close(infopfd[0]);
            dup2(infopfd[1],1);
            if(execlp("./mmap_read","./mmap_read",(char *)0)<0) 
                fprintf(stderr, "exec error\n");
            fprintf(stderr, "exec error\n" );
        }
        //pause();
