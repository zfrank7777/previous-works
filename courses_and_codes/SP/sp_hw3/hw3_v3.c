/*
endof v3: <info> after fork(),using child to send package to conn_fd.
*/
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
...
//// HANDLE INFO ////    
        ...
        if(infopid==0){ //child
            close(infopfd[1]);
        //read parent's msg from [1][0], write to [0][1]
            //send signal
            //fprintf(stderr, "before kill()\n" );
            kill(getppid(),SIGUSR1);
            //fprintf(stderr, "after kill:%d\n",SIGUSR1 );
            //read pipe
            //write to conn_fd
            reqP->buf_len = 0;
            memset(reqP->buf,reqP->buf_size,0);
            ...
            add_to_buf( reqP, buf, buflen );        
            //fprintf(stderr, "INFO PRINT BUF:\n%s",reqP->buf );
            char frompipe[MAXBUFSIZE];
            read(infopfd[0],frompipe,MAXBUFSIZE);
        ...
        //read parent's msg from [0][0], write to [1][1]
            return 200;
        }
