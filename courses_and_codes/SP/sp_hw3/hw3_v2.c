...
//// HANDLE READFILE ////
    ...
    if(strncmp(query,checkcommand,9)==0){
        ...
        add_to_buf( reqP, buf, buflen );
       // buflen = snprintf(buf, sizeof(buf), "Content-Length: 0\015\012" );
       // add_to_buf( reqP, buf, buflen );
        buflen = snprintf( buf, sizeof(buf), "Connection: close\015\012\015\012" );
        add_to_buf( reqP, buf, buflen );

        buflen = read(readfd,buf,sizeof(buf));
        add_to_buf( reqP, buf, buflen );
