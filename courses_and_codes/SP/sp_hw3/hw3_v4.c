/*
v4:
child exit after sending SIGUSR1 to parent. 
*/
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
...
static void sighandler(int signum){
    ...
    if(signum==SIGUSR1 && infopid!=0){
        getsig_flag=1;
        printf("SIGUSR1 recieved(parent)\n");
        int len;
        ...
        if(FD_ISSET(server.listen_fd,&temp)){
        	...
        	while (1) {
        		...
        		// write once only and ignore error
                    ...
                    //fprintf(stderr, "ret=400,conn_fd=%d\n",conn_fd );
                    search_connfd[1000]=conn_fd;
                    infopid=fork();
                    if(infopid==0){
                        kill(getppid(),SIGUSR1);
                        fprintf(stderr, "SIGUSR1 send\n" );
                        exit(0);       
                    }   
                    break;            
                }
                    ...
                    printf("add fdset to listen:pipe=%d\n",ret);
            ...
            if(CGIstat==0){
                //fprintf(stderr, "%d\n", tempconnfd);         
                requestP[tempconnfd].buf_len = 0;
                memset(requestP[tempconnfd].buf,requestP[tempconnfd].buf_size,0);
                buflen = snprintf( buf, sizeof(buf), "HTTP/1.1 400 OK\015\012Server: SP TOY\015\012" );
...
//// HANDLE 400 ////
    ...
    if(strncmp(query,checkcommand,9)==0 && strcmp(file,"file_reader")==0){
        pipe(rpfd);
        int pid = fork();
        if(pid==0){
            close(rpfd[0]);
            ...
            if(execlp("./mmap_read","./mmap_read",pwdfilename,(char *)0)<0) 
        ...
        /*
            ...
            buflen = read(rpfd[0],buf,sizeof(buf));
            add_to_buf( reqP, buf, buflen );
            //fprintf(stderr, "PRINT BUF:\n%s",reqP->buf );
        */
       }
    }
