...
void merge(int x,int y,int relation,int n){
	sets[group[x]].tail=sets[group[y]].head;
...
int main(){
	...
	for(int x=0;x<T;x++){
		...
		buf_index=0;
		for(int i=0;i<M;i++){
			sets[i].head=&buffer[buf_index];
			buf_index++;
			sets[i].tail=sets[i].head;
			sets[i].Group_num=0;
		}		
		for(int i=0;i<N+1;i++)
		...
		for(int i=0;i<Q;i++){
			...
			if(R=='R'){
				...
				if(group[x]==0 && group[y]==0){
					...
					printf("O\n");
					//printf("QQ\n");
					*(sets[group_index].tail)=x;
					sets[group_index].tail++;
					*(sets[group_index].tail)=y;
					sets[group_index].tail++;
